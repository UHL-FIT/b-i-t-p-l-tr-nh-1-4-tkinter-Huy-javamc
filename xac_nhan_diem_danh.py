...
from tkinter import messagebox  # Để hiện cửa sổ thông báo lỗi
...
def xu_ly_du_lieu():
    ...
    # 2.0 Ràng buộc dữ liệu: Kiểm tra rỗng và MSSV phải là số
    if ho_ten == "" or mssv == "":
        messagebox.showwarning("Chú ý", "Vui lòng không để trống thông tin sinh viên!")
        return # Dừng hàm, không xử lý tiếp
    if not mssv.isdigit(): # Kiểm tra MSSV có phải toàn số không
        messagebox.showerror("Lỗi dữ liệu", "Mã sinh viên phải là định dạng số!")
        return # Dừng hàm
    # 2 In ra Terminal để lập trình viên kiểm tra
    #   print(f"Dữ liệu nhận được: MSSV: {mssv} - Họ tên: {ho_ten}")
    
    # 3.0 Tương tác Terminal: In kèm thời gian
    thoi_gian_hien_tai = datetime.now().strftime("%H:%M:%S")
    print(f"[{thoi_gian_hien_tai}] Đã nhận: MSSV {mssv} - Tên: {ho_ten}")
